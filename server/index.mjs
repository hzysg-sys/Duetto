...
const DEFAULTS = {
  ...
  room_sub: '',
  relationship_start: process.env.DUETTO_RELATIONSHIP_START || '2026-06-03',
  ai: {
    base_url: '',
    api_key: '',
    model: '',
...
app.get('/api/config',(_q,r)=>{ const s=getSettings(); r.json({ok:true,config:{companion:{name:s.ai_name,has_key:Boolean(s.ai.api_key),model:s.ai.model},user:{display_name:s.user_name},room:{title:s.room_name,subtitle:s.room_sub,relationship_start:s.relationship_start}}}); });
...
function ncmSessionKey(){ const a=readAuth(); const secret=String((a&&a.secret)||envAuthSecret||''); return secret?crypto.createHash('sha256').update('duetto-ncm-session:'+secret).digest():null; }
function sealNcmCookie(v){
  try{ const key=ncmSessionKey(); if(!key||!v) return ''; const iv=crypto.randomBytes(12); const cipher=crypto.createCipheriv('aes-256-gcm',key,iv); const body=Buffer.concat([cipher.update(String(v),'utf8'),cipher.final()]); const tag=cipher.getAuthTag(); return Buffer.concat([Buffer.from([1]),iv,tag,body]).toString('base64url'); }catch(e){ return ''; }
}
function unsealNcmCookie(token){
  try{ const key=ncmSessionKey(); if(!key||!token||String(token).length>16384) return ''; const raw=Buffer.from(String(token),'base64url'); if(raw.length<30||raw[0]!==1) return ''; const decipher=crypto.createDecipheriv('aes-256-gcm',key,raw.subarray(1,13)); decipher.setAuthTag(raw.subarray(13,29)); const out=Buffer.concat([decipher.update(raw.subarray(29)),decipher.final()]).toString('utf8'); return out.length<=12000?out:''; }catch(e){ return ''; }
}
function restoreNcmCookie(req){ const restored=unsealNcmCookie(req.headers['x-duetto-ncm']); if(restored&&restored!==ncmCookie){ saveNcmCookie(restored); console.log('[ncm/session] restored encrypted session'); } }
app.use('/api/ncm',(req,_res,next)=>{ restoreNcmCookie(req); next(); });
...
app.get('/api/ncm/check', async (q,r)=>{ try{ const key=q.query.key; const c=await ncm.login_qr_check({ key }); const code=c.body.code; if(code===803){ saveNcmCookie(c.body.cookie); const p=await ncmProfile(); const session=sealNcmCookie(ncmCookie); console.log('[ncm/session] connected',{uid:p&&p.userId,session:Boolean(session)}); r.json({ ok:true, code, logged:true, nickname:p&&p.nickname, avatar:p&&p.avatarUrl, uid:p&&p.userId, session }); } else { r.json({ ok:true, code, message:c.body.message||'' }); } }catch(e){ r.status(500).json({ok:false,error:String(e.message||e)}); } });
app.get('/api/ncm/status', async (_q,r)=>{ const p=await ncmProfile(); if(p) r.json({ ok:true, logged:true, nickname:p.nickname, avatar:p.avatarUrl, uid:p.userId }); else r.json({ ok:true, logged:false, needs_reconnect:!ncmCookie }); });
...
app.get('/api/ncm/song-url', async (q,r)=>{ try{ if(!ncmCookie) return r.json({ok:false,url:'',needs_reconnect:true,error:'网易云需要重新连接'}); const su=await ncm.song_url_v1({ id:q.query.id, level:'standard', cookie:ncmCookie }); const u=su.body&&su.body.data&&su.body.data[0]; let url=u&&u.url||''; if(url) url=url.replace(/^http:/,'https:'); if(!url) console.log('[ncm/song-url] empty',{id:String(q.query.id||''),code:u&&u.code}); r.json({ ok:true, url }); }catch(e){ console.error('[ncm/song-url] failed',{id:String(q.query.id||''),error:String(e.message||e)}); r.status(500).json({ok:false,error:String(e.message||e)}); } });
